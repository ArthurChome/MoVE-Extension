...
object Global {
  ...
  lazy val encoding: Charset = Charset.forName("UTF-8")
  ...
  lazy val shortcuts: ShortCutHandler = new ShortCutHandler(getConfigFile("shortcuts.conf"))
  lazy val config: Config = new ConfigLoader(getConfigFile("move.conf"))
  lazy val recentFilesPath: Path = configDirPath.resolve("recent-files.json")

  lazy val historySize: Int = Global.config.getInt("history.cache-size").getOrElse(50)
  lazy val history = new History(historySize)

  lazy val copyright: String = build.ProjectInfo.copyright
  lazy val version: String = build.ProjectInfo.version

  lazy val licenseFile = "/LICENSE"

  lazy val fontBundle: CustomResourceBundle = new CustomResourceBundle(List("fonts/fontawesome", "i18n/messages"), Locale.getDefault)
  lazy val styleSheetUrl: String = MoveApp.getClass.getResource("/stylesheets/style.css").toExternalForm

  lazy val licenseString: String =
