...
class ResizablePolygon(private val points:List[Double])
  ...
  with VertexLike
{
  ...
  override def localVertexes:List[Point] = {
    getPoints.sliding(2, 2).map(x => (x.head.toDouble, x.tail.head.toDouble)).toList
  }
}
