...
object Dialogs {
  ...
  val jsonFileFilter = new FileChooser.ExtensionFilter("Json files (*.json)", "*.json")
  ...
  def newSnapPointSensitivityDialog(cellSize:Int)(implicit marshaller:StringMarshaller[Int]): InputDialog[Int] = {
    val dialog = new InputDialog(fontBundle.getString("pointsensitivitydialog.inputfield") -> Some(cellSize))
    dialog.setTitle(fontBundle.getString("pointsensitivitydialog.title"))
    dialog.setHeaderText(fontBundle.getString("pointsensitivitydialog.header"))
    dialog
  }
  ...
  def newJsonFileChooser(): FileChooser =
    newFileChooser(jsonFileFilter)(allFilesFilter, jsonFileFilter)
