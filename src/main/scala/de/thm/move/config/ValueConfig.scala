...
/**
  * Retrieves configuration options from a .conf file
  * @param url
  */
class ValueConfig(url:URL) {
  val values: ArrayBuffer[String] = Source.fromURL(url, Global.encoding.name).getLines().to[mutable.ArrayBuffer]
  ...
  def setUniqueValue(v:String): Unit = if(!values.contains(v)) setValue(v)
  def setValue(v:String): values.type = values += v
