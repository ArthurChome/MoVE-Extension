...
/** Behaviour for colorizing selected ResizableShapes.
  * That is, changing the fill colour, stroke colour, stroke width */
trait ColorizeSelectionCtrl {
  ...
  def setFillColor(color:Color): Unit = if(getSelectedShapes.nonEmpty) {
  ...
  def setStrokeColor(color:Color): Unit = if(getSelectedShapes.nonEmpty) {
