...
import de.thm.move.controllers.drawing.MeasurementStrategy
import java.net.URI
import de.thm.move.controllers.drawing.WallSnapping.{DoorStrategy, WindowStrategy}
...
class DrawCtrl(changeLike:ChangeDrawPanelLike) {
  ...
  //Define all strategies.
   private val drawStrategies =
      Map(SelectedShape.RectangleRoom -> new RectangleRoomStrategy(changeLike),
        ...
        SelectedShape.Polygon -> new PolygonStrategy(changeLike),
          // Extend with measurement option.
          // Strategy has to be defined later.
        SelectedShape.Measurement -> new MeasurementStrategy(changeLike),
        SelectedShape.PolygonRoom -> new PolygonRoomStrategy(changeLike),
        SelectedShape.Window -> new WindowStrategy(changeLike),
        SelectedShape.Door -> new DoorStrategy(changeLike))

  private val tmpShapeId = DrawPanel.tmpShapeId

  /** The constraint that indicates if the drawing-element should hold their
    * proportions. E.g. a rectangle becomes a square,...
    */
