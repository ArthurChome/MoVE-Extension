...
class SelectedShapeCtrl(
  ...
  //Experiment
  override var Like: ChangeDrawPanelLike = changeLike
  ...
  def deleteSelectedShape(): Unit = {
    if(selectedShapes.nonEmpty) {
        val shapeCopy = selectedShapes
        //Holds the shapes of deleted House objects
        var deletedRooms: List[Room] = List()
        var deletedWindows: List[Window] = List()
        var deletedDoors: List[Door] = List()
        var deletedMeasurements: List[Measurement] = List()
        history.execute {
          shapeCopy foreach { shape =>
            changeLike.remove(shape.selectionRectangle)
            shape.rotationAnchors.map(_.getId).foreach(changeLike.removeById)
            House.current.rooms().foreach(room => if (room.shape == shape) {deletedRooms = room :: deletedRooms})
            House.current.measurements().foreach(measurement => if (measurement.group == shape) {deletedMeasurements = measurement :: deletedMeasurements})
            House.current.windows().foreach(window => if (window.shape == shape) {deletedWindows = window :: deletedWindows})
            House.current.doors().foreach(door => if (door.shape == shape) {deletedDoors = door :: deletedDoors})
            changeLike.removeShape(shape)
          }
          deletedRooms.foreach(House.current.remRoom)
          deletedDoors.foreach(House.current.remDoor)
          deletedWindows.foreach(House.current.remWindow)
          deletedMeasurements.foreach(House.current.remMeasurement)
          ...
          deletedRooms.foreach(House.current.addRoom)
          deletedDoors.foreach(House.current.addDoor)
          deletedMeasurements.foreach(House.current.addMeasurement)
          deletedWindows.foreach(House.current.addWindow)
        }
        selectedShapes = List()
    }
  }
  ...
  def getGroupSelectionHandler: MouseEvent => Unit = {
    //Pair representing the X and Y coordinate of the mouse.
    var mouseP = (0.0,0.0)
    ...
    //This function handles the mouse input on the interface.
