...
class ContextMenuCtrl(drawPanel:DrawPanel, changeLike:ChangeDrawPanelLike) {
  ...
  private def newContextMenu(underlyingElement:ResizableShape):ShapeContextMenu = {
    ...
    menu.deleteElementItem.setOnAction(onDeleteClicked(underlyingElement) _)
    ...
    //Setion to make the foundation out of a figure.
    menu.makeFoundation.setOnAction(onMakeFoundationsPressed(underlyingElement)_)
  ...
  private def onMakeFoundationsPressed(underlyingElement:Node)(ae:ActionEvent): Unit = {
  }
  ...
  //Function for when you want to delete an element with the contextmenu.
  def onDeleteClicked(shape:ResizableShape)(ae:ActionEvent):Unit = {
